...
struct NoticeService {
    ...
    func notice(completion: @escaping (NetworkResult<Any>) -> Void) {
        ...
        let dataRequest = Alamofire.request(APIConstants.noticeURL, method: .get, encoding: JSONEncoding.default, headers: header)
    ...
    private func isUser(by data: Data) -> NetworkResult<Any> {
        ...
        guard let decodedData = try? decoder.decode(NoticeData.self, from: data) else { return .pathErr }
